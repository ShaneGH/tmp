import { PropertyKeyword, Property, Properties, LazyTypeReference, BinaryType, BinaryTypeCombinator, AliasedType, PropertyType } from "./types";
...
class LazyStringDictionary<T> extends LazyDictionary<T> {
    buildKey(x: string) { return x; }
};

function serialize(value: AliasedType[]) {

    const result = new LazyStringDictionary<WrapperKind<any>>();
...
function deserialize(values: {[key: string]: WrapperKind<any>}) {
    ...
    const result = new LazyStringDictionary<AliasedType>();
